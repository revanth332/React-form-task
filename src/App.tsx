...
const schema = z.object({
  ...
  password: z.string().refine((pwd) => {
    const pattern = /^(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{10,}$/;
    return pattern.test(pwd)
  },"Password didnt meet the criteria"),
  dob: z.string().refine((val) => !isNaN(Date.parse(val)) && (Date.parse(val) < Date.now()), "Invalid date"),
  color: z.string(),
    ...
    >
      ...
      <DialogContent className="sm:max-w-[425px]">
        ...
        <div className="grid gap-4 py-4">
          ...
          <Button variant="outline" onClick={() => handleFileType(fileTypes[0].value)}>Image</Button>
          <Button variant="outline" onClick={() => handleFileType(fileTypes[1].value)}>Pdf</Button>
          <Button variant="outline" onClick={() => handleFileType(fileTypes[2].value)}>Text</Button>
        </div>
        </div>
      </DialogContent>
    </Dialog>
